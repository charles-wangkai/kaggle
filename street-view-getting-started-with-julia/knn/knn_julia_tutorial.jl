...
yTrain = convert(Array, map(Int, yTrain))
...
labelsInfoTest[:Class] = convert(Array, map(Char, yPredictions))

#Save predictions
writetable("juliaKNNSubmission.csv", labelsInfoTest, separator=',', header=true)

println("Submission file saved in juliaKNNSubmission.csv")
